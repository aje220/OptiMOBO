...
if __name__ == "__main__":
    ...
    # prob = MyProblem()
    # # optimiser = 
    # # optimiser = EMO(ideal_point=[1,1], max_point=[8,9], n_obj=2, )
    # optimiser = EMO(prob, [0,0], [700,12])
    # # import cProfile
    # # import pstats
    # # profiler = cProfile.Profile()
    # # profiler.enable()

    # out = optimiser.solve(n_iterations=10, n_init_samples=10)
    ...
    problem_string = "dtlz5"
    # problem = get_problem(problem_string, n_var=5)
    ref_point = None
    if problem_string in {"dtlz2", "dtlz4", "dtlz5"}:
        ref_point = np.array([1.3,1.3])
    elif problem_string in {"zdt1", "zdt2", "zdt3"}:
        ref_point = np.array([1.5, 12.0])
    elif problem_string in {"zdt4"}:
        ref_point = np.array([5.0, 30.0])
    elif problem_string in {"zdt6"}:
        ref_point = np.array([1.1,12.5])
    elif problem_string in {"dtlz6"}:
        ref_point = np.asarray([5.0,5.0])
    elif problem_string in {"dtlz7"}:
        ref_point = np.asarray([3.0, 10.0])
    else:
        print("ERRRRR")
    ...
    optimi = EMO(problem, [0,0], ref_point)
    out1 = optimi.solve(n_iterations=30, n_init_samples=30)
    out1.plot_pareto_front()
    import pdb; pdb.set_trace()
    plt.show()
    out1.plot_hv_convergence()
    plt.show()
